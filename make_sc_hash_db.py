...
def apHash(inString, fName):
    ...
    for b in inString:
        ...
        if 0x41 <= b <= 0x5A:
            b += 0x20
        if toggle:
            h = h ^ (~(((h << 11) & 0xffffffff) ^ (h >> 5) ^ b) & 0xffffffff)
            toggle = False
        else:
            h = h ^ (((h << 7) & 0xffffffff) ^ (h >> 3) ^ b)
            toggle = True
        h &= 0xffffffff
    return h & 0xffffffff

pseudocode_apHash = '''acc := 0;
toggle := false;
for c in input_string {
    if c == 0:
        break;
    if 'A' <= c <= 'Z':
        c := c + 0x20;
    if toggle {
        acc := acc ^ ~(((acc << 11) ^ (acc >> 5) ^ c) & 0xffffffff);
        toggle := false;
    } else {
        acc := acc ^ (((acc << 7) ^ (acc >> 3) ^ c) & 0xffffffff);
        toggle := true;
    }
    acc := acc & 0xffffffff;
}
